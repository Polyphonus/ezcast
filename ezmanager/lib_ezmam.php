...
function ezmam_album_course_public_name_get($album)
{
    ...
    } elseif (isset($album_metadata['name'])) {
 ...
 * @param string $album
 * @return false|assoc_array
 * @desc returns metadata from the album original asset
 */
function ezmam_album_orig_metadata_get($album,$asset)
{
    $repository_path = ezmam_repository_path();
    if ($repository_path === false) {
    ...
    $album_path = $repository_path . "/" . $album;
    $assoc_metadata = metadata2assoc_array($album_path ."/".$asset."/original_cam/_metadata.xml");
    ...
    foreach ($assets as $asset) {
        ...
        if ($type == 'ezplayer') {
            ...
            if ($metadata['record_type'] == 'camslide') {
                ...
                $arguments = '?action=media&album=' . $album . '&asset=' . $asset['name'] . '&type=cam&quality=' . $type . '&token=' . $token . "&origin=rss";
                ...
                // Slides
                $arguments = '?action=media&album=' . $album . '&asset=' . $asset['name'] . '&type=slide&quality=' . $type . '&token=' . $token . "&origin=rss";
            ...
            // Only add one video otherwise
                ...
                $title = (isset($metadata['title'])) ? $metadata['title'] : $metadata['description'];
                $arguments = '?action=media&album=' . $album . '&asset=' . $asset['name'] . '&type=' . $metadata['record_type'] . '&quality=' . $type . '&token=' . $token . "&origin=rss";
 ...
 * @param string $album_name
 * @param string $type May be "low" or "high" or "ezplayer"
 ...
 */
function add_item_to_rss(&$xmlh, $title, $description, $author, $pubDate, $link = false, $media_link = false)
{
...
function ezmam_asset_delete($album_name, $asset_time, $rebuild_rss = true)
{
    ...
    if (!ezmam_asset_exists($album_name, $asset_time)) {
        ezmam_last_error("Trying to delete $asset_time which doesn't exist!");
        return false;
    }

    $path = $repository_path . '/' . $album_name . '/' . $asset_time;
    ...
    while (($file = readdir($dir)) !== false) {
        if ($file != "." && $file != "..") {
            if (is_dir($path . '/' . $file)) {
                ezmam_media_delete($album_name, $asset_time, $file);
            } else {
                unlink($path . '/' . $file);
            }
        }
...
function ezmam_copy_or_move_get_paths($asset_time, $album_src, $album_dst, &$src_path, &$dst_path)
{
    // Sanity checks
    $repository_path = ezmam_repository_path();
    if ($repository_path === false) {
...
function ezmam_asset_move($asset_time, $album_src, $album_dst)
{
    ...
    ezmam_rss_generate($album_src, "ezplayer");
...
function ezmam_asset_copy($asset_time, $album_src, $album_dst)
{
    ...
    if ($res != 0) {
        ...
        $logger->log(EventType::MANAGER_ASSET_COPY, LogLevel::ERROR, "Failed to start copying asset: $asset_time, from: $album_src, to $album_dst. Cmd: $final_cmd", array(basename(__FUNCTION__)), $asset_name);
        return false;
    }
    // Logging
...
function ezmam_media_new($album_name, $asset_name, $media_name, $metadata, $media_file_path, $copy = false)
{
    ...
    if (!ezmam_asset_exists($album_name, $asset_name)) {
        ezmam_last_error("ezmam_media_new asset [$asset_name] non existant");
        return false;
    }
...
function ezmam_media_viewcount_increment($album, $asset, $media, $origin = '')
{
    // Sanity checks
    $repository_path = ezmam_repository_path();
    if ($repository_path == false) {
    ...
    if (!isset($byte_range) || $byte_range == '' || $byte_range === 'bytes=0-' || $byte_range == 'bytes=0-1') {
        ...
        switch ($origin) {
            case 'ezcast':
                $viewcount_file = dirname(ezmam_media_getpath($album, $asset, $media, false)) . '/_ezcast_count';
                break;
            case 'ezplayer':
                $viewcount_file = dirname(ezmam_media_getpath($album, $asset, $media, false)) . '/_ezplayer_count';
                break;
            case 'rss':
                $viewcount_file = dirname(ezmam_media_getpath($album, $asset, $media, false)) . '/_rss_count';
                break;
            case 'link':
                $viewcount_file = dirname(ezmam_media_getpath($album, $asset, $media, false)) . '/_link_count';
                break;
            case 'embed':
                $viewcount_file = dirname(ezmam_media_getpath($album, $asset, $media, false)) . '/_embed_count';
                break;
            default:
