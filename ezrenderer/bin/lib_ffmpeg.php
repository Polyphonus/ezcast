...
function movie_encode($moviein, $movieout, $encoder, $qtinfo, $letterboxing = true) {
    global $ffmpegpath, $encoders_path, $aac_experimental,$gpu_enabled;
    // sanity check
    if (!is_file($moviein))
        return "input movie not found $moviein";
    // safe path
    ...
    //if($gpu_enabled ){
       // $video_filter = "scale_npp=$width:$height";
        $video_filter = "scale=$width:$height";

        
       // $cmd = "$ffmpegpath -y -hwaccel cuvid -c:v h264_cuvid -vsync 0 -i $moviein -r 25 $start -fpre $encoder -vf \"hwdownload,format=nv12,format=yuv444p,hwupload_cuda\" -vf $video_filter -vcodec h264_nvenc -rc vbr_hq -b:v 8M -maxrate:v 10M -y $aac_codec $movieout";
...
function generateVideoFromSound($audioin,$movieout,$imagePath){    
    ...
    file_put_contents('/home/arwillame/test_intro/test21.txt', ($audioin).PHP_EOL.($audioout).PHP_EOL.json_encode($path_parts),FILE_APPEND);
