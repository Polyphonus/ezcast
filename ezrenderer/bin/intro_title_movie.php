...
file_put_contents('/home/arwillame/test_intro/test1.txt', json_encode($toprocess_assoc),FILE_APPEND);
...
file_put_contents('/home/arwillame/test_intro/test6.txt', json_encode($toprocess_assoc),FILE_APPEND);
...
file_put_contents('/home/arwillame/test_intro/test6.txt', json_encode($originals),FILE_APPEND);
...
function itm_intro_title_movie($camslide, $moviein, &$title_assoc, $intro, $add_title, $credits) {
    ...
    // file_put_contents('/home/ezrenderer-gpu/ezrenderer/bin/tetsttttttt.txt','realMimetype : '.mime_content_type($moviein) .'expected....'.json_encode($toprocess_assoc["record_type"]). PHP_EOL,FILE_APPEND);
...
//    
   ...
   if($enableMimeTypeCheck && $toprocess_assoc["record_type"]=='audio' && !in_array(mime_content_type($moviein),$audio_mimeTypes) ){
    ...
    // file_put_contents('/home/arwillame/test_intro/tetsttttttt4.txt','IN config : '.json_encode($toprocess_assoc). PHP_EOL,FILE_APPEND);
file_put_contents('/home/arwillame/test_intro/test3.txt',json_encode($toprocess_assoc),FILE_APPEND);

//    generate video from sound submited and image
    if($toprocess_assoc["record_type"]=="audio" ){
        file_put_contents('/home/arwillame/test_intro/test4.txt', json_encode($toprocess_assoc),FILE_APPEND);

        
        $movieout= $processing .'/cam.mp4';
        $audioin=$moviein;
        // file_put_contents('/home/ezrenderer-gpu/ezrenderer/bin/tetsttttttt2.txt','IN IT : '. PHP_EOL,FILE_APPEND);
//        file_put_contents('/home/arwillame/test_intro/test22.txt', "AAAAA3:". json_encode(generateVideoFromSound($audioin,$movieout,$imageAudioFilePath)).PHP_EOL.PHP_EOL,FILE_APPEND);
